//! Row-batched `f32` layer normalization for row-major matrices.
//!
//! Each row is transformed as
//! `output = (input - mean) * rsqrt(variance + epsilon) * gamma + beta`.
//!
//! Two execution paths share one CUDA source module:
//!
//! - staged: one block reduces each row's statistics into device columns,
//!   then an elementwise kernel applies the affine normalization
...
extern "C" __global__ void nnis_layernorm_row_stats_f32(
    const float* input,
    float* row_mean,
    float* row_var,
    ...
    value = 0.0f;
    for (unsigned long long index = lane; index < cols; index += blockDim.x) {
        const float centered = row_input[index] - mean;
    ...
    if (lane == 0) {
        row_mean[row] = mean;
        row_var[row] = partial[0];
    }
}

extern "C" __global__ void nnis_layernorm_normalize_f32(
    const float* input,
    float* output,
    ...
    if (index < count) {
        ...
        const float variance = row_var[row] * inv_cols;
        const float scale = rsqrtf(variance + epsilon);
        output[index] = fmaf((input[index] - row_mean[row]) * scale, gamma, beta);
    }
}
...
extern "C" __global__ void nnis_layernorm_row_fused_f32(
    const float* input,
    float* output,
    unsigned long long cols,
    float inv_cols,
    ...
    const float mean = partial[0] * inv_cols;
    __syncthreads();
    ...
    for (unsigned long long index = lane; index < cols; index += blockDim.x) {
        destination[index] = fmaf(values[index] * scale, gamma, beta);
    }
}
"#;
...
/// Context-bound row-batched `f32` layer normalization.
///
/// A [`F32LayerNormWorkspace`] holds the device-side per-row statistic
/// columns and may be reused across non-overlapping calls with equal or
/// smaller row counts.
#[derive(Debug)]
...
impl F32LayerNorm {
    /// Compile (or reuse cached CUBIN) and load the layer-norm kernel set.
    ...
    pub fn load_with_block_size(
        ...
        if block_size == 0 || !block_size.is_power_of_two() {
            return Err(NnisError::invalid_input(format!(
                "layer-norm block size {block_size} is not a non-zero power of two"
            )));
        }
        ...
        let code =
            compiler.compile_cubin(LAYER_NORM_SOURCE, &CompileOptions::for_device(context))?;
        let module = Module::load(context, &code)?;
        let row_stats = module.get_function("nnis_layernorm_row_stats_f32")?;
        let normalize = module.get_function("nnis_layernorm_normalize_f32")?;
        let fused = module.get_function("nnis_layernorm_row_fused_f32")?;
        for (name, function) in [
            ...
            if block_size > attributes.max_threads_per_block {
                return Err(NnisError::invalid_input(format!(
                    "layer-norm {name} block size {block_size} exceeds function limit {}",
            ...
            {
                return Err(NnisError::invalid_input(format!(
                    "layer-norm {name} requires {shared_memory_bytes} shared-memory bytes; \
                     function limit is {}",
    ...
    pub fn workspace(
        ...
        if !Arc::ptr_eq(context, self.context()) {
            return Err(NnisError::invalid_input(
                "layer-norm and workspace contexts do not match",
            ));
        }
    ...
    pub fn normalize_rows(
        ...
        gamma: f32,
        beta: f32,
        workspace: &F32LayerNormWorkspace,
    ) -> Result<()> {
    ...
    pub fn fused_normalize_rows(
        ...
        // SAFETY: every buffer borrow remains live until synchronization.
        let enqueue_result = unsafe {
            self.enqueue_fused_rows(stream, input, output, rows, cols, epsilon, gamma, beta)
        };
        match enqueue_result {
            Ok(()) => stream.synchronize(),
            Err(error) => {
    ...
    pub unsafe fn enqueue_fused_rows(
        ...
        if shared_memory_bytes > attributes.max_dynamic_shared_memory_bytes as usize {
            return Err(NnisError::invalid_input(format!(
                "fused layer norm needs {shared_memory_bytes} shared-memory bytes for \
                 {cols} columns; function limit is {}",
                attributes.max_dynamic_shared_memory_bytes
            )));
        }
        // SAFETY: the caller owns the asynchronous lifetime of both buffers.
        unsafe { self.launch_fused(stream, input, output, rows, cols, epsilon, gamma, beta) }
    }
    ...
    fn fused_shared_memory(&self, cols: usize) -> Result<usize> {
        ...
        let values = element
            .checked_mul(cols)
            .ok_or_else(|| NnisError::invalid_input("fused layer norm shared size overflows"))?;
    ...
    fn validate_shapes(
        ...
        let count = rows
            .checked_mul(cols)
            .ok_or_else(|| NnisError::invalid_input("layer-norm shape overflows usize"))?;
        if input.len() != count {
            return Err(NnisError::invalid_input(format!(
                "layer-norm input has {} elements; shape ({rows}, {cols}) requires {count}",
                input.len()
            )));
        }
        if output.len() != count {
            return Err(NnisError::invalid_input(format!(
                "layer-norm output has {} elements; shape ({rows}, {cols}) requires {count}",
                output.len()
            )));
        }
        ...
        {
            return Err(NnisError::invalid_input(
                "layer-norm stream, buffers, and kernels must share one context",
            ));
        }
        Ok(())
    }
    ...
    fn validate_execution(
        ...
        if rows > workspace.max_rows {
            return Err(NnisError::invalid_input(format!(
                "layer-norm has {rows} rows; workspace capacity is {}",
                workspace.max_rows
            )));
        }
    ...
    unsafe fn launch_row_stats(
        ...
        let grid_size = u32::try_from(rows)
            .map_err(|_| NnisError::invalid_input("layer-norm exceeds u32::MAX rows"))?;
        let cols = u64::try_from(cols)
            .map_err(|_| NnisError::invalid_input("layer-norm width exceeds u64::MAX"))?;
    ...
    unsafe fn launch_normalize(
        ...
        // `nnis_layernorm_normalize_f32`; same-stream ordering guarantees
        // each row's statistics are final before use.
        unsafe { launch.launch(&mut arguments) }
    }

    fn u64_shape(&self, rows: usize, cols: usize) -> Result<(u64, u64)> {
        let rows = u64::try_from(rows)
            .map_err(|_| NnisError::invalid_input("layer-norm exceeds u64::MAX rows"))?;
        let cols = u64::try_from(cols)
            .map_err(|_| NnisError::invalid_input("layer-norm width exceeds u64::MAX"))?;
        Ok((rows, cols))
    }
    ...
    ///
    /// Caller owns the asynchronous lifetime of every buffer; see
    /// [`F32LayerNorm::enqueue_fused_rows`].
    #[allow(clippy::too_many_arguments)]
    unsafe fn launch_fused(
        ...
        let mut arguments = KernelArgs::with_capacity(7, 2);
        arguments
            .push_buffer(input)
            .push_buffer(output)
            .push(cols_u64)
        ...
        // SAFETY: argument order/widths match
        // `nnis_layernorm_row_fused_f32`; the caller owns the asynchronous
        // lifetime obligation.
        unsafe { launch.launch(&mut arguments) }
    }

    fn context(&self) -> &Arc<Context> {
        self.row_stats.context()
    }
}
...
mod tests {
    ...
    fn reference_rows(input: &[f32], rows: usize, cols: usize) -> Vec<f64> {
        input
            ...
            .flat_map(|slice| {
                ...
                let mean: f64 = slice.iter().map(|&value| f64::from(value)).sum::<f64>() / count;
                let variance: f64 = slice
                    ...
                    .sum::<f64>()
                    / count;
                let scale = (variance + f64::from(EPSILON)).sqrt().recip();
                slice.iter().map(move |&value| {
                    let normalized = (f64::from(value) - mean) * scale;
                    normalized * f64::from(GAMMA) + f64::from(BETA)
                })
            })
            .collect()
    }
    ...
    #[test]
    fn layer_norm_matches_high_precision_oracle_on_gpu() {
        ...
        let workspace = layer_norm.workspace(&context, max_rows).unwrap();
        ...
        // A constant row centers to zero: every output equals beta plus
        // floating-point noise from the mean subtraction.
        let (rows, cols) = (6_usize, 129_usize);
        let input =
            DeviceBuffer::from_host(&context, &stream, &vec![3.5_f32; rows * cols]).unwrap();
        let output = DeviceBuffer::<f32>::new(&context, rows * cols).unwrap();
        let small_workspace = layer_norm.workspace(&context, rows).unwrap();
        layer_norm
            .normalize_rows(
                &stream,
                &input,
    ...
    #[test]
    fn fused_layer_norm_matches_high_precision_oracle_on_gpu() {
        let Some(context) = gpu_context() else {
            eprintln!("skipped: no CUDA device");
            return;
        };
    ...
    #[test]
    fn fused_layer_norm_rejects_oversized_rows() {
        let Some(context) = gpu_context() else {
            eprintln!("skipped: no CUDA device");
            return;
        };
        ...
        for &(rows, cols, fused_expected) in &[
            ...
            layer_norm
                .normalize_rows_dispatched(
                    &stream, &input, &output, rows, cols, EPSILON, GAMMA, BETA,
                )
                .unwrap();
            let actual = output.to_vec(&stream).unwrap();
            assert_close(
            ...
            assert_eq!(fused_expected, layer_norm.fused_available(cols));
        }
    }

    #[test]
    fn layer_norm_rejects_invalid_shapes_before_launch() {
        let Some(context) = gpu_context() else {
            eprintln!("skipped: no CUDA device");
            return;
        };
        let compiler = JitCompiler::new();
        assert!(F32LayerNorm::load_with_block_size(&context, &compiler, 0).is_err());
        assert!(F32LayerNorm::load_with_block_size(&context, &compiler, 300).is_err());
        let layer_norm = F32LayerNorm::load(&context, &compiler).unwrap();
        let stream = Stream::new(&context).unwrap();
        let input = DeviceBuffer::<f32>::new(&context, 16).unwrap();
        let short_output = DeviceBuffer::<f32>::new(&context, 10).unwrap();
        let workspace = layer_norm.workspace(&context, 3).unwrap();
        let error = layer_norm
            .normalize_rows(
                &stream,
                &input,
        ...
        let error = layer_norm
            .normalize_rows(
                &stream, &input, &output, 4, 4, EPSILON, GAMMA, BETA, &workspace,
            )
            .unwrap_err();
        assert!(
