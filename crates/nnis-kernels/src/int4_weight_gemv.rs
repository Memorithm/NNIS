//! Batch-one projection with F32 activations and packed signed-INT4 weights.
//!
//! This primitive is deliberately narrow: it consumes the versioned NNIS
//! reference packing layout directly, dequantizes one weight at a time in a
//! register, and accumulates into F32 without materializing a dense weight
//! buffer. It is an isolated execution primitive, not a full-model INT4
//! promotion or a performance claim.
...
const SOURCE: &str = r#"
__device__ __forceinline__ int nnis_decode_signed_int4(unsigned char packed, unsigned int high) {
    const unsigned int nibble = high ? ((unsigned int)packed >> 4) : ((unsigned int)packed & 15u);
    return (nibble & 8u) ? ((int)nibble - 16) : (int)nibble;
}

extern "C" __global__ void nnis_project_kn_f32_int4_weight(
    const float* input,
    ...
    for (unsigned long long row = 0; row < k; ++row) {
        ...
        const unsigned char packed = packed_weight[flat >> 1];
        const int q = nnis_decode_signed_int4(packed, (unsigned int)(flat & 1ull));
...
/// Context-bound `[1,K] × [K,N] -> [1,N]` projection using F32 activations,
/// packed signed-INT4 weights, one resident F32 scale and F32 accumulation.
///
/// The kernel accepts the full signed nibble domain `[-8, 7]` safely. The
/// NNIS INT4 reference-storage contract is stricter and never emits `-8`.
/// Storage validation therefore remains the authority for representation
/// semantics; this kernel owns only typed shape/context execution.
#[derive(Debug)]
pub struct F32Int4Gemv {
    project_kn: Kernel,
    block_size: u32,
}

impl F32Int4Gemv {
    /// Compile and load the default INT4 projection primitive.
    ...
    pub fn load_with_block_size(
        ...
        if block_size == 0 || !block_size.is_power_of_two() {
            return Err(NnisError::invalid_input(format!(
                "f32-int4 project block size {block_size} is not a non-zero power of two"
            )));
        }
        let code = compiler.compile_cubin(SOURCE, &CompileOptions::for_device(context))?;
        let module = Module::load(context, &code)?;
        let project_kn = module.get_function("nnis_project_kn_f32_int4_weight")?;
        let attributes = project_kn.attributes()?;
        if block_size > attributes.max_threads_per_block {
            return Err(NnisError::invalid_input(format!(
                "f32-int4 project block size {block_size} exceeds function limit {}",
    ...
    /// Enqueue one packed-INT4 projection without synchronizing.
    ///
    /// # Safety
    ///
    ...
    pub unsafe fn enqueue_project_kn(
        ...
        let k_arg = u64::try_from(k)
            .map_err(|_| NnisError::invalid_input("f32-int4 project K exceeds u64::MAX"))?;
        let n_arg = u64::try_from(n)
            .map_err(|_| NnisError::invalid_input("f32-int4 project N exceeds u64::MAX"))?;
    ...
    fn validate_execution(
        ...
        let logical_weight_elements = k
            .checked_mul(n)
            .ok_or_else(|| NnisError::invalid_input("f32-int4 project shape overflows usize"))?;
        let expected_packed = logical_weight_elements
            .checked_add(1)
            .ok_or_else(|| NnisError::invalid_input("f32-int4 packed length overflows usize"))?
            / 2;
        if input.len() != k {
            return Err(NnisError::invalid_input(format!(
                "f32-int4 project input has {} elements; shape requires {k}",
                input.len()
            )));
        }
        if packed_weight.len() != expected_packed {
            return Err(NnisError::invalid_input(format!(
                "f32-int4 project packed weight has {} bytes; shape ({k}, {n}) requires {expected_packed}",
                packed_weight.len()
            )));
        }
        if scale.len() != 1 {
            return Err(NnisError::invalid_input(format!(
                "f32-int4 project scale buffer has {} values; exactly one is required",
                scale.len()
            )));
        }
        if output.len() != n {
            return Err(NnisError::invalid_input(format!(
                "f32-int4 project output has {} elements; shape requires {n}",
                output.len()
            )));
        }
            ...
            || !Arc::ptr_eq(context, output.ctx())
        {
            return Err(NnisError::invalid_input(
                "f32-int4 project stream, buffers and kernel must share one context",
            ));
        }
        Ok(())
    }
...
mod tests {
    ...
    #[test]
    fn int4_projection_matches_quantized_ordered_cpu_oracle_on_gpu() {
        ...
        let kernel = F32Int4Gemv::load_with_block_size(&context, &compiler, 64).unwrap();
        ...
        for (index, (&actual, &expected)) in actual.iter().zip(&expected).enumerate() {
            assert_eq!(
                actual.to_bits(),
                expected.to_bits(),
                "INT4 ordered projection mismatch at output {index}: actual={actual}, expected={expected}"
            );
        }
    }

    #[test]
    fn int4_projection_rejects_shape_contract_violations_before_launch() {
        let Some(context) = gpu_context() else {
            eprintln!("skipped: no CUDA device");
            return;
        };
        let compiler = JitCompiler::new();
        assert!(F32Int4Gemv::load_with_block_size(&context, &compiler, 0).is_err());
        assert!(F32Int4Gemv::load_with_block_size(&context, &compiler, 48).is_err());

        let kernel = F32Int4Gemv::load(&context, &compiler).unwrap();
        let stream = Stream::new(&context).unwrap();
        let input = DeviceBuffer::<f32>::new(&context, 4).unwrap();
        let packed = DeviceBuffer::<u8>::new(&context, 6).unwrap();
        ...
        let error = kernel
            ...
            .unwrap_err();
        assert!(error.to_string().contains("requires 8"), "{error}");

        let packed = DeviceBuffer::<u8>::new(&context, 8).unwrap();
