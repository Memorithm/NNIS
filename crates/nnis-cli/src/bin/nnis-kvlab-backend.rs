...
const REQUEST_SCHEMA: &str = "kvlab.prospect-kv-backend-request/v3";
const RESPONSE_SCHEMA: &str = "kvlab.prospect-kv-backend-response/v3";
const TRACE_SCHEMA: &str = "kvlab.prospect-kv-real-model-trace/v1";
const ARTIFACT_SCHEMA: &str = "nnis.kvlab-teacher-forced-evaluation/v1";
const DEFAULT_RUNTIME_BACKEND: &str = "nnis-kvlab-v3";

const USAGE: &str = "Usage:\n  nnis-kvlab-backend --model DIR --model-id ID --model-revision REV --tokenizer-revision REV --runtime-revision REV [--runtime-backend ID] [--device N]\n\nReads one canonical KVLab backend request v3 from stdin and writes one canonical response v3 to stdout. The first evaluation token is a bridge token processed after any candidate KV compaction; metrics score only the remaining teacher-forced evaluation tokens.";
...
struct RequestV3 {
    ...
    trace_sha256: String,
    seed: u64,
    logical_input_token_ids: Vec<u64>,
    model_input_token_ids: Vec<u32>,
    retained_logical_token_ids: Vec<u64>,
...
fn parse_request(payload: &str, args: &Args) -> Result<RequestV3, String> {
    let request: RequestV3 =
        serde_json::from_str(payload).map_err(|error| format!("invalid request JSON: {error}"))?;
    validate_request(&request, args)?;
    Ok(request)
}

fn validate_request(request: &RequestV3, args: &Args) -> Result<(), String> {
    ...
    if request.model_input_token_ids.len() != request.logical_input_token_ids.len() {
        return Err(
            "model_input_token_ids must be position-aligned with logical_input_token_ids"
                .to_string(),
        );
    }
    ...
    let retained_set: BTreeSet<u64> = request.retained_logical_token_ids.iter().copied().collect();
    let canonical_retained: Vec<u64> = request
        .logical_input_token_ids
        .iter()
        .copied()
        .filter(|identity| retained_set.contains(identity))
        .collect();
    if canonical_retained != request.retained_logical_token_ids {
        return Err(
            "retained_logical_token_ids must be an ordered subset of logical_input_token_ids"
                .to_string(),
        );
    }
    ...
    match request.mode.as_str() {
        "baseline" => {
            ...
            if request.retained_logical_token_ids != request.logical_input_token_ids {
                return Err("baseline must retain every logical input row".to_string());
            }
        }
        "candidate" => {
            let policy = request
    ...
    if expected_trace_sha256 != request.trace_sha256 {
        return Err("request trace_sha256 does not match embedded real-model trace".to_string());
    }
    Ok(())
}

fn validate_unique_ids(name: &str, values: &[u64], allow_empty: bool) -> Result<(), String> {
    if !allow_empty && values.is_empty() {
        return Err(format!("{name} must not be empty"));
    }
    let unique: BTreeSet<u64> = values.iter().copied().collect();
    if unique.len() != values.len() {
        return Err(format!("{name} must contain unique logical ids"));
    }
    Ok(())
}

fn trace_sha256(request: &RequestV3) -> Result<String, String> {
    ...
    trace.insert(
        "logical_input_token_ids".to_string(),
        serde_json::to_value(&request.logical_input_token_ids)
            .map_err(|error| format!("failed to serialize logical ids: {error}"))?,
    );
    trace.insert(
...
fn retained_rows(request: &RequestV3) -> Vec<usize> {
    let retained: BTreeSet<u64> = request.retained_logical_token_ids.iter().copied().collect();
    request
        .logical_input_token_ids
        .iter()
        .enumerate()
        .filter_map(|(index, identity)| retained.contains(identity).then_some(index))
        .collect()
}

fn execute_request(args: &Args, request: &RequestV3) -> Result<(Vec<u8>, f64, f64), String> {
    ...
    let requested_positions = request
        ...
        .len()
        .checked_add(request.evaluation_token_ids.len())
    ...
    if requested_positions > model.config().max_position_embeddings {
        return Err(format!(
            "model input plus evaluation requires {requested_positions} logical positions but model capacity is {}",
    ...
    let rows = retained_rows(request);
    if request.mode == "candidate" {
        session
            .compact_kv_cache_rows(&rows)
    ...
    let artifact = render_artifact(request, &rows, position_after_selection, bridge, &steps)?;
...
fn render_artifact(
    ...
    root.insert(
        "retained_logical_token_ids".to_string(),
        serde_json::to_value(&request.retained_logical_token_ids)
            .map_err(|error| format!("failed to serialize retained identities: {error}"))?,
    );
    root.insert(
        "retained_physical_rows".to_string(),
...
fn render_response(
    ...
    root.insert(
        "applied_retained_logical_token_ids".to_string(),
        serde_json::to_value(&request.retained_logical_token_ids)
            .map_err(|error| format!("failed to serialize applied retained identities: {error}"))?,
    );
    root.insert(
        "metrics".to_string(),
        Value::Array(vec![
...
mod tests {
    ...
    fn args() -> Args {
        Args {
            ...
            runtime_revision: "a69734c33b26fac85810ef4ce63502fb7a65f228".to_string(),
            device_ordinal: 0,
        }
    }

    fn request_json() -> String {
        let logical = vec![10_u64, 11, 12, 13, 14];
        ...
        request.insert(
            "retained_logical_token_ids".to_string(),
            serde_json::to_value(vec![10_u64, 12, 14]).unwrap(),
        );
        request.insert(
    ...
    #[test]
    fn request_maps_repeated_model_tokens_to_unique_logical_rows() {
        let payload = request_json();
        let request = parse_request(&payload, &args()).unwrap();
        assert_eq!(request.model_input_token_ids, [42, 7, 42, 9, 7]);
        assert_eq!(retained_rows(&request), [0, 2, 4]);
    }

    #[test]
    fn trace_hash_detects_mapping_drift() {
        let payload = request_json();
    ...
    fn response_is_sorted_canonical_ascii_json() {
        ...
        assert!(response.contains("kvlab.prospect-kv-backend-response/v3"));
    }
}
