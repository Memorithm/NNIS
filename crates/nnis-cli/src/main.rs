use nnis::{
    current_process_gpu_memory, reference_weight_capability_manifest_v1,
    ElasticStageBPreregistrationHandoffV1, Context, Device,
...
#[derive(Debug, PartialEq)]
struct QualifiedWeightCapabilitiesArgs {
    input: PathBuf,
    tokenizer: PathBuf,
    json: bool,
}
...
enum Command {
    ...
    QualifiedWeightCapabilities(QualifiedWeightCapabilitiesArgs),
    ...
    I: IntoIterator<Item = String>,
{
    ...
    if command == "qualified-weight-capabilities" {
        return parse_qualified_weight_capabilities_args(arguments);
    }
    if command == "qualified-weight-capabilities-v2" {
        return parse_qualified_weight_capabilities_v2_args(arguments);
    }
    if command == "qualified-weight-handoff" {
        return parse_qualified_weight_handoff_args(arguments);
    }
    ...
    I: IntoIterator<Item = String>,
{
    ...
    match command {
        Command::QualifiedWeightCapabilities(arguments) => {
            Ok(Command::ElasticStageBHandoff(arguments))
        }
        Command::Help => Ok(Command::Help),
        _ => Err("internal elastic-stage-b-handoff parser mismatch".to_string()),
    }
}

fn parse_qualified_weight_handoff_args<I>(arguments: I) -> Result<Command, String>
where
    I: IntoIterator<Item = String>,
{
    let command = parse_qualified_weight_capabilities_args(arguments)?;
    match command {
        Command::QualifiedWeightCapabilities(arguments) => {
            Ok(Command::QualifiedWeightHandoff(arguments))
        }
        Command::Help => Ok(Command::Help),
        _ => Err("internal qualified-weight-handoff parser mismatch".to_string()),
    }
}

fn parse_qualified_weight_capabilities_v2_args<I>(arguments: I) -> Result<Command, String>
where
    I: IntoIterator<Item = String>,
{
    let command = parse_qualified_weight_capabilities_args(arguments)?;
    match command {
        Command::QualifiedWeightCapabilities(arguments) => {
            Ok(Command::QualifiedWeightCapabilitiesV2(arguments))
        }
        Command::Help => Ok(Command::Help),
        _ => Err("internal qualified-weight-capabilities-v2 parser mismatch".to_string()),
    }
}

fn parse_qualified_weight_capabilities_args<I>(arguments: I) -> Result<Command, String>
where
    I: IntoIterator<Item = String>,
{
    let mut input = None;
    ...
    while let Some(argument) = arguments.next() {
        match argument.as_str() {
            ...
            other => {
                return Err(format!(
                    "unknown qualified-weight-capabilities argument {other:?}\n\n{USAGE}"
                ))
            }
        }
    }
    Ok(Command::QualifiedWeightCapabilities(
        QualifiedWeightCapabilitiesArgs {
...
fn qualified_weight_handoff(arguments: &QualifiedWeightCapabilitiesArgs) -> Result<String, String> {
    ...
    artifact
        .verify_tokenizer_file(&arguments.tokenizer)
        .map_err(|error| format!("tokenizer verification failed: {error}"))?;
    let handoff = QualifiedWeightHandoffV1::from_artifact(&artifact)
        .map_err(|error| format!("failed to derive qualified weight handoff: {error}"))?;
    if arguments.json {
        serde_json::to_string_pretty(&handoff)
            .map_err(|error| format!("failed to serialize qualified weight handoff: {error}"))
    } else {
        Ok(format!(
            "NNIS qualified weight handoff valid\nartifact_sha256: {}\nnnis_commit: {}\nexact_checkpoint: {}\ntokenizer_sha256: {}\nfamily_records: {}\ndevice_uuid: {}\nsm_arch: {}\ncuda_driver: {}.{}\nelastic_stage_b_backend_ready: true",
            handoff.artifact_fingerprint.artifact_json_sha256,
            handoff.qualified_capability.capability.nnis_commit,
            handoff.qualified_capability.capability.exact_checkpoint,
            handoff.qualified_capability.capability.tokenizer_sha256,
            handoff.qualified_capability.capability.families.len(),
            handoff.qualified_capability.environment.device_uuid,
            handoff.qualified_capability.environment.sm_arch,
            handoff.qualified_capability.environment.cuda_driver_major,
            handoff.qualified_capability.environment.cuda_driver_minor
        ))
    }
}

fn qualified_weight_capabilities_v2(
    arguments: &QualifiedWeightCapabilitiesArgs,
) -> Result<String, String> {
    let raw = fs::read_to_string(&arguments.input).map_err(|error| {
        format!(
    ...
    artifact
        .verify_tokenizer_file(&arguments.tokenizer)
        .map_err(|error| format!("tokenizer verification failed: {error}"))?;
    let record = QualifiedWeightCapabilityRecordV2::from_artifact(&artifact)
        .map_err(|error| format!("failed to derive qualified weight capability v2: {error}"))?;
    if arguments.json {
        serde_json::to_string_pretty(&record)
            .map_err(|error| format!("failed to serialize qualified capability v2: {error}"))
    } else {
        Ok(format!(
            "NNIS qualified weight capabilities v2\nnnis_commit: {}\nexact_checkpoint: {}\ntokenizer_sha256: {}\nfamily_records: {}\ndevice_uuid: {}\nsm_arch: {}\ncuda_driver: {}.{}\nelastic_stage_b_backend_ready: true",
            record.capability.nnis_commit,
            record.capability.exact_checkpoint,
            record.capability.tokenizer_sha256,
            record.capability.families.len(),
            record.environment.device_uuid,
            record.environment.sm_arch,
            record.environment.cuda_driver_major,
            record.environment.cuda_driver_minor
        ))
    }
}

fn qualified_weight_capabilities(
    arguments: &QualifiedWeightCapabilitiesArgs,
) -> Result<String, String> {
    let raw = fs::read_to_string(&arguments.input).map_err(|error| {
        format!(
    ...
    artifact
        .verify_tokenizer_file(&arguments.tokenizer)
        .map_err(|error| format!("tokenizer verification failed: {error}"))?;
    let record = QualifiedWeightCapabilityRecordV1::from_artifact(&artifact)
        .map_err(|error| format!("failed to derive qualified weight capability record: {error}"))?;
    if arguments.json {
        serde_json::to_string_pretty(&record)
            .map_err(|error| format!("failed to serialize qualified capability record: {error}"))
    } else {
        Ok(format!(
            "NNIS qualified weight capabilities\nnnis_commit: {}\nexact_checkpoint: {}\ntokenizer_sha256: {}\nfamily_records: {}\nelastic_stage_b_backend_ready: true",
            record.nnis_commit,
            record.exact_checkpoint,
...
fn main() -> ExitCode {
    ...
    match command {
        ...
        Command::QualifiedWeightCapabilities(arguments) => {
            match qualified_weight_capabilities(&arguments) {
                Ok(rendered) => {
                    println!("{rendered}");
                    ExitCode::SUCCESS
                }
                Err(error) => {
                    eprintln!("nnis qualified-weight-capabilities: {error}");
                    ExitCode::FAILURE
                }
            }
        }
        Command::QualifiedWeightCapabilitiesV2(arguments) => {
            match qualified_weight_capabilities_v2(&arguments) {
                Ok(rendered) => {
                    println!("{rendered}");
                    ExitCode::SUCCESS
                }
                Err(error) => {
                    eprintln!("nnis qualified-weight-capabilities-v2: {error}");
                    ExitCode::FAILURE
                }
            }
        }
        Command::QualifiedWeightHandoff(arguments) => match qualified_weight_handoff(&arguments) {
            Ok(rendered) => {
                println!("{rendered}");
                ExitCode::SUCCESS
            }
            Err(error) => {
                eprintln!("nnis qualified-weight-handoff: {error}");
                ExitCode::FAILURE
            }
        },
        Command::ElasticStageBHandoff(arguments) => match elastic_stage_b_handoff(&arguments) {
            Ok(rendered) => {
                println!("{rendered}");
                ExitCode::SUCCESS
            }
            Err(error) => {
                eprintln!("nnis elastic-stage-b-handoff: {error}");
                ExitCode::FAILURE
            }
        },
...
mod tests {
    ...
    #[test]
    fn elastic_stage_b_handoff_parser_requires_artifact_and_tokenizer() {
        assert_eq!(
            parse_args(strings(&[
                "elastic-stage-b-handoff",
                "--input",
            ...
            .unwrap(),
            Command::ElasticStageBHandoff(QualifiedWeightCapabilitiesArgs {
    ...
    #[test]
    fn qualified_weight_handoff_parser_requires_artifact_and_tokenizer() {
        assert_eq!(
            parse_args(strings(&[
                "qualified-weight-handoff",
                "--input",
                "/tmp/artifact-v2.json",
                "--tokenizer",
                "/tmp/tokenizer.json",
            ...
            .unwrap(),
            Command::QualifiedWeightHandoff(QualifiedWeightCapabilitiesArgs {
                input: PathBuf::from("/tmp/artifact-v2.json"),
                tokenizer: PathBuf::from("/tmp/tokenizer.json"),
                json: true,
            })
    ...
    #[test]
    fn qualified_weight_capabilities_v2_parser_requires_artifact_and_tokenizer() {
        assert_eq!(
            parse_args(strings(&[
                "qualified-weight-capabilities-v2",
                "--input",
                "/tmp/artifact-v2.json",
                "--tokenizer",
                "/tmp/tokenizer.json",
            ...
            .unwrap(),
            Command::QualifiedWeightCapabilitiesV2(QualifiedWeightCapabilitiesArgs {
                input: PathBuf::from("/tmp/artifact-v2.json"),
                tokenizer: PathBuf::from("/tmp/tokenizer.json"),
                json: true,
            })
    ...
    #[test]
    fn qualified_weight_capabilities_parser_requires_artifact_and_tokenizer() {
        assert_eq!(
            parse_args(strings(&[
                "qualified-weight-capabilities",
                "--input",
                "/tmp/artifact.json",
                "--tokenizer",
                "/tmp/tokenizer.json",
            ...
            .unwrap(),
            Command::QualifiedWeightCapabilities(QualifiedWeightCapabilitiesArgs {
