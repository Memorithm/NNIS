...
fn limits() -> F32GraphLimitsV1 {
    F32GraphLimitsV1 {
        max_inputs: 16, max_nodes: 32, max_tensor_bytes: 1024,
        max_live_payload_bytes: 8192, max_scratch_bytes: 1024, max_work_items: 100000,
    }
}

fn plan<'a>(inputs: &'a [F32ShapeV1], nodes: &'a [F32NodeV1<'a>]) -> F32GraphV1<'a> {
    F32GraphV1 { schema_version: F32_GRAPH_VERSION, numerical_policy: F32_GRAPH_POLICY, inputs, nodes }
}
...
fn tensor(values: &[f32]) -> CpuBuffer {
    ...
    let mut buffer = device.create_buffer(BufferDesc::new(data.len() as u64, usage, MemoryClass::Host).unwrap()).unwrap();
    device.create_queue().unwrap().write_buffer(&mut buffer, 0, &data).unwrap();
    buffer
}

fn bytes(buffer: &CpuBuffer) -> Vec<u8> {
    CpuDevice::new().unwrap().create_queue().unwrap()
        .read_buffer(buffer, 0, buffer.len() as u64).unwrap()
}
...
fn validated_seven_node_graph_executes_and_can_be_reused_with_new_bindings() {
    ...
    let nodes = [
        F32NodeV1 { operation: F32OpV1::ProjectKn { input: 0, weights: 1 }, output: F32ShapeV1::Vector(2) },
        F32NodeV1 { operation: F32OpV1::Add { left: 5, right: 2 }, output: F32ShapeV1::Vector(2) },
        F32NodeV1 { operation: F32OpV1::Relu { input: 6 }, output: F32ShapeV1::Vector(2) },
        F32NodeV1 { operation: F32OpV1::ProjectKn { input: 7, weights: 3 }, output: F32ShapeV1::Vector(2) },
        F32NodeV1 { operation: F32OpV1::Add { left: 8, right: 4 }, output: F32ShapeV1::Vector(2) },
    ...
    for (input_values, expected) in [([2.0, -1.0, 3.0], 5.0), ([0.0; 3], 15.0)] {
        ...
        let result = execute_f32_graph(&device, &valid, &[&input, &weights1, &bias1, &weights2, &bias2]).unwrap();
...
fn scatter_add_creates_a_new_value_and_preserves_base_and_source() {
    ...
    let nodes = [F32NodeV1 {
        operation: F32OpV1::ScatterAdd { base: 0, source: 1, indices: &[1, 1, 0] },
...
fn branched_dataflow_can_read_one_prior_value_multiple_times() {
    ...
    let nodes = [
        F32NodeV1 { operation: F32OpV1::Relu { input: 0 }, output: shapes[0] },
        F32NodeV1 { operation: F32OpV1::Multiply { left: 1, right: 1 }, output: shapes[0] },
        F32NodeV1 { operation: F32OpV1::Add { left: 1, right: 2 }, output: shapes[0] },
    ];
    let graph = plan(&shapes, &nodes).validate(limits()).unwrap();
...
fn late_arithmetic_failure_returns_no_result_and_preserves_all_inputs() {
    ...
    let nodes = [
        F32NodeV1 { operation: F32OpV1::Relu { input: 0 }, output: shapes[0] },
        F32NodeV1 { operation: F32OpV1::Add { left: 1, right: 1 }, output: shapes[0] },
    ];
    let graph = plan(&shapes, &nodes).validate(limits()).unwrap();
...
fn binding_count_length_usage_and_unused_nonfinite_inputs_are_rejected() {
    ...
    let nodes = [F32NodeV1 { operation: F32OpV1::Relu { input: 0 }, output: shapes[0] }];
    let graph = plan(&shapes, &nodes).validate(limits()).unwrap();
    let device = CpuDevice::new().unwrap();
    ...
    let no_storage = device.create_buffer(BufferDesc::new(4, BufferUsages::COPY_SRC, MemoryClass::Host).unwrap()).unwrap();
...
fn backend_capabilities_are_rechecked_after_portable_validation() {
    ...
    assert!(execute_f32_graph(&CpuDevice::with_max_buffer_bytes(7).unwrap(), &graph, &[&input]).is_err());
    let result = execute_f32_graph(&CpuDevice::with_max_buffer_bytes(8).unwrap(), &graph, &[&input]).unwrap();
...
fn failed_unused_node_is_not_silently_eliminated() {
    ...
    let nodes = [
        F32NodeV1 { operation: F32OpV1::Multiply { left: 0, right: 0 }, output: shapes[0] },
        F32NodeV1 { operation: F32OpV1::Relu { input: 0 }, output: shapes[0] },
    ];
    let graph = plan(&shapes, &nodes).validate(limits()).unwrap();
...
fn aliased_input_bindings_are_read_only_and_conservatively_charged() {
    ...
    let nodes = [F32NodeV1 { operation: F32OpV1::Add { left: 0, right: 1 }, output: shapes[0] }];
    let graph = plan(&shapes, &nodes).validate(limits()).unwrap();
