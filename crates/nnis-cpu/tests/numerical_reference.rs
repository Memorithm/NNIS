...
fn raw(bytes: &[u8], storage: bool) -> CpuBuffer {
    ...
    device.create_queue().unwrap().write_buffer(&mut buffer, 0, bytes).unwrap();
    buffer
}
...
fn binary_operations_preserve_inputs_and_output_capacity() {
    ...
    let report = kernels().binary(CpuF32BinaryOp::Add, &left, &right, &mut output).unwrap();
    ...
    kernels().binary(CpuF32BinaryOp::Multiply, &left, &right, &mut output).unwrap();
...
fn sum_is_serial_and_does_not_reassociate() {
    ...
    kernels().sum(&tensor(&[16_777_216.0, 1.0, -16_777_216.0]), &mut output).unwrap();
    assert_values(&output, &[0.0]);
    kernels().sum(&tensor(&[16_777_216.0, -16_777_216.0, 1.0]), &mut output).unwrap();
...
fn projection_respects_kn_orientation_on_rectangular_matrix() {
    ...
    kernels().project_kn(&input, &weights, &mut output, 3, 2).unwrap();
...
fn projection_uses_one_fused_rounding_not_split_multiply_add() {
    ...
    kernels().project_kn(&input, &weight, &mut output, 2, 1).unwrap();
...
fn late_projection_overflow_preserves_entire_output() {
    ...
    assert!(kernels().project_kn(&input, &weight, &mut output, 2, 2).is_err());
...
fn overflowing_partial_sum_is_not_rescued_by_later_cancellation() {
    ...
    assert!(kernels().sum(&tensor(&[f32::MAX, f32::MAX, -f32::MAX]), &mut output).is_err());
...
fn binary_overflow_is_atomic_even_after_valid_first_element() {
    ...
    for operation in [CpuF32BinaryOp::Add, CpuF32BinaryOp::Multiply] {
        assert!(kernels().binary(operation, &tensor(&[1.0, f32::MAX]), &tensor(&[2.0, f32::MAX]), &mut output).is_err());
        assert_eq!(bytes(&output), before);
    }
}
...
fn nonfinite_inputs_are_rejected_including_unselected_gather_values() {
    for value in [f32::NAN, f32::INFINITY, f32::NEG_INFINITY] {
        ...
        assert!(kernels().binary(CpuF32BinaryOp::Add, &input, &tensor(&[1.0; 2]), &mut vector).is_err());
        assert!(kernels().sum(&input, &mut scalar).is_err());
        assert!(kernels().gather(&input, &[0], &mut scalar).is_err());
        assert!(kernels().project_kn(&tensor(&[1.0]), &input, &mut vector, 1, 2).is_err());
...
fn shapes_and_dimension_overflow_fail_before_output_changes() {
    ...
    for (k, n) in [(0, 1), (1, 0), (usize::MAX, 2), (1, 2), (2, 2)] {
        assert!(kernels().project_kn(&input, &weight, &mut output, k, n).is_err());
        assert_values(&output, &[42.0]);
    }
    assert!(kernels().binary(CpuF32BinaryOp::Add, &input, &tensor(&[1.0]), &mut output).is_err());
    assert!(kernels().relu(&input, &mut output).is_err());
    assert_values(&output, &[42.0]);
}
...
fn scratch_limit_is_explicit_and_exact_boundary_is_accepted() {
    ...
    assert!(CpuF32KernelsV1::new(7).unwrap().relu(&input, &mut output).is_err());
    assert_values(&output, &[42.0; 2]);
    let exact = CpuF32KernelsV1::new(8).unwrap();
    assert_eq!(exact.max_scratch_bytes(), 8);
    assert_eq!(exact.relu(&input, &mut output).unwrap().scratch_payload_bytes, 8);
...
fn scatter_add_duplicates_follow_input_order_and_preserve_other_values() {
    ...
    kernels().scatter_add(&source, &[0, 0, 0], &mut output).unwrap();
    assert_values(&output, &[0.0, -0.0, 7.0]);
    kernels().scatter_add(&tensor(&[2.0, 3.0]), &[2, 0], &mut output).unwrap();
...
fn scatter_errors_preserve_destination_after_partial_staging() {
    ...
    let before = bytes(&output);
    assert!(kernels().scatter_add(&tensor(&[1.0, f32::MAX]), &[0, 1], &mut output).is_err());
    assert_eq!(bytes(&output), before);
    assert!(kernels().scatter_add(&tensor(&[1.0, 1.0]), &[0, 2], &mut output).is_err());
    assert_eq!(bytes(&output), before);
    assert!(CpuF32KernelsV1::new(4).unwrap().scatter_add(&tensor(&[1.0]), &[0], &mut output).is_err());
    assert_eq!(bytes(&output), before);
    assert!(kernels().scatter_add(&tensor(&[1.0]), &[], &mut output).is_err());
    assert_eq!(bytes(&output), before);
}
...
fn scatter_rejects_nonfinite_existing_destination_but_overwrite_can_replace_it() {
    ...
    let before = bytes(&output);
    assert!(kernels().scatter_add(&tensor(&[1.0]), &[1], &mut output).is_err());
    assert_eq!(bytes(&output), before);
...
fn memory_copy_size_overflow_remains_rejected_without_mutation() {
    ...
    assert!(queue.copy_buffer(&source, 0, &mut destination, 0, u64::MAX).is_err());
