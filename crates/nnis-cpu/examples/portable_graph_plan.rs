...
fn run() -> Result<()> {
    ...
    let limits = F32GraphLimitsV1 {
        ...
        max_nodes: 7,
        max_tensor_bytes: 24,
        max_live_payload_bytes: 136,
        max_scratch_bytes: 12,
        max_work_items: 1000,
    };
    if plan
        .validate(F32GraphLimitsV1 {
            max_live_payload_bytes: 135,
            ..limits
        })
        .is_ok()
    {
        return Err(PortableError::Backend(
            "undersized graph budget accepted".to_string(),
        ));
    }
    ...
    if data != 5.0_f32.to_le_bytes() {
        return Err(PortableError::Backend(
            "graph plan output mismatch".to_string(),
        ));
    }
    println!(
