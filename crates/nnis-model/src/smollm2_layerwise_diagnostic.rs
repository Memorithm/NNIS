...
fn smollm2_prefill_layerwise_diagnostic_on_gpu() {
    ...
    for layer_index in 0..model.config.num_hidden_layers {
        ...
        if layer_index == 24 {
            ...
            let reference_projected =
                expected_stage(&manifest, &reference_dir, "layer24.attention_projected");
            let o_weights = layer
                .o_proj
                .tensor()
                .as_f32()
            ...
            let ordered_from_nnis =
                ordered_projection(&nnis_pre_o, &o_weights, model.config.hidden_size);
            let ordered_from_reference =
                ordered_projection(&reference_pre_o, &o_weights, model.config.hidden_size);
            report_host_vectors(
