...
const NODES: [F32NodeV1<'static>; 2] = [
    F32NodeV1 {
        operation: F32OpV1::ProjectKn { input: 0, weights: 1 },
...
fn exact_limits_are_inclusive_and_each_smaller_limit_fails() {
    let exact = F32GraphLimitsV1 {
        max_inputs: 2, max_nodes: 2, max_tensor_bytes: 16,
        max_live_payload_bytes: 48, max_scratch_bytes: 8, max_work_items: 24,
    };
    assert!(plan(&INPUTS, &NODES).validate(exact).is_ok());
    for smaller in [
        F32GraphLimitsV1 { max_inputs: 1, ..exact },
        F32GraphLimitsV1 { max_nodes: 1, ..exact },
        F32GraphLimitsV1 { max_tensor_bytes: 15, ..exact },
        F32GraphLimitsV1 { max_live_payload_bytes: 47, ..exact },
        F32GraphLimitsV1 { max_scratch_bytes: 7, ..exact },
        F32GraphLimitsV1 { max_work_items: 23, ..exact },
    ] {
...
fn zero_limits_are_never_interpreted_as_unlimited() {
    let standard = limits();
    for zero in [
        F32GraphLimitsV1 { max_inputs: 0, ..standard },
        F32GraphLimitsV1 { max_nodes: 0, ..standard },
        F32GraphLimitsV1 { max_tensor_bytes: 0, ..standard },
        F32GraphLimitsV1 { max_live_payload_bytes: 0, ..standard },
        F32GraphLimitsV1 { max_scratch_bytes: 0, ..standard },
        F32GraphLimitsV1 { max_work_items: 0, ..standard },
    ] {
...
fn public_literal_versions_and_policy_are_revalidated() {
    ...
    assert!(F32GraphV1 { schema_version: 2, ..original }.validate(limits()).is_err());
    assert!(F32GraphV1 { numerical_policy: "fast-math", ..original }.validate(limits()).is_err());
...
fn output_shape_and_vector_matrix_kind_must_match_exactly() {
    for shape in [F32ShapeV1::Vector(1), F32ShapeV1::Matrix { rows: 1, cols: 2 }] {
        let mut nodes = NODES;
...
fn zero_dimensions_and_tensor_byte_overflow_are_errors() {
    for shape in [
        F32ShapeV1::Vector(0), F32ShapeV1::Vector(u64::MAX),
        F32ShapeV1::Matrix { rows: 0, cols: 2 },
        F32ShapeV1::Matrix { rows: u64::MAX, cols: 2 },
    ] {
...
fn input_total_overflow_cannot_wrap_under_a_memory_limit() {
    ...
    let nodes = [F32NodeV1 { operation: F32OpV1::Sum { input: 0 }, output: F32ShapeV1::Vector(1) }];
    let large = F32GraphLimitsV1 {
        max_tensor_bytes: u64::MAX, max_live_payload_bytes: u64::MAX,
        max_work_items: u64::MAX, ..limits()
    };
...
fn gather_checks_whole_index_list_and_accepts_duplicates() {
    ...
    let accepted = [1, 0, 1];
    let node = F32NodeV1 {
        operation: F32OpV1::Gather { input: 0, indices: &accepted },
...
fn scatter_binds_base_source_and_indices_without_in_place_output() {
    ...
    let mut nodes = [F32NodeV1 {
        operation: F32OpV1::ScatterAdd { base: 0, source: 1, indices: &indices },
        output: F32ShapeV1::Vector(2),
    }];
    assert!(plan(&inputs, &nodes).validate(limits()).is_ok());
    nodes[0].operation = F32OpV1::ScatterAdd { base: 0, source: 1, indices: &[0] };
    assert!(plan(&inputs, &nodes).validate(limits()).is_err());
    nodes[0].operation = F32OpV1::ScatterAdd { base: 2, source: 1, indices: &indices };
    assert!(plan(&inputs, &nodes).validate(limits()).is_err());
}
