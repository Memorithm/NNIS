...
fn parse_arguments() -> std::result::Result<Arguments, String> {
    ...
    while let Some(argument) = args.next() {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                return Err("usage: smollm2_f16_combined_candidate_e2e --model DIR [--device N] [--warmups N] [--iterations N]".to_string());
            }
...
fn require_expected_profile(profile: &F16ReferenceGenerationProfile) -> Result<()> {
    if profile.generated_ids.as_slice() != EXPECTED_GREEDY_IDS {
        return Err(NnisError::invalid_input(format!(
            "combined F16 candidate greedy trajectory changed: actual={:?} expected={:?}",
    ...
    {
        return Err(NnisError::invalid_input(format!(
            "combined F16 candidate metric contract drifted: {profile:?}"
        )));
    }
    Ok(())
}
...
fn run(arguments: Arguments) -> Result<Report> {
    ...
    let attention_plan = F16AttentionPlan::thor_ka17_parallel_score_candidate();
            ...
            != Some(F16ParallelScorePolicy::Ka17SmolLm2ShortContextV1)
    {
        return Err(NnisError::unsupported(
            "combined SmolLM2/Thor F16 candidate did not preserve its selected plans",
        ));
    }
    ...
    for _ in 0..arguments.iterations {
        ...
        if let Some(expected) = &generated_ids {
            if profile.generated_ids != *expected {
                return Err(NnisError::invalid_input(
                    "greedy output changed across combined F16 candidate iterations",
                ));
            }
        } else {
    ...
    if !gpu_median.is_finite() || gpu_median <= 0.0 {
        return Err(NnisError::unsupported(
            "combined F16 candidate generation GPU median is non-positive",
        ));
    }

    Ok(Report {
        schema_version: 1,
        benchmark: "smollm2-135m-f16-combined-candidate-e2e",
        backend: "nnis",
        plan_name: "fused_mlp_plus_ka17_parallel_score_candidate_v1",
        promotion_state: "combined candidate; component plans were qualified separately and require combined physical requalification",
        ...
        limitations: vec![
            "fused_mlp and KA17 attention were qualified separately and their combination is not promoted by component evidence alone",
            "the KA17 attention selector is officially qualified only with the transposed-projection parent used by KA18/KA19",
            "short-context attention policy is measured only through KV row 35 and falls back to reference outside its declared domain",
            "physical combined end-to-end evidence is required before any combined selector can be promoted",
        ],
    })
}
...
fn main() {
    ...
    match run(arguments) {
        Ok(report) => match serde_json::to_string_pretty(&report) {
            ...
            Err(error) => {
                eprintln!("failed to serialize combined F16 candidate report: {error}");
                std::process::exit(1);
            }
        },
        Err(error) => {
            eprintln!("combined SmolLM2 F16 candidate benchmark failed: {error}");
            std::process::exit(1);
        }
    }
}
