...
fn expected_outputs(input: &[f32], rows: usize, cols: usize) -> impl Iterator<Item = f64> + '_ {
    input.chunks(cols).take(rows).flat_map(move |slice| {
        ...
        slice.iter().map(move |&value| {
            (f64::from(value) - mean) * scale * f64::from(GAMMA) + f64::from(BETA)
        })
    })
}
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let bytes = (rows as u64)
        ...
        .ok_or("layer norm traffic calculation overflow")?;

    let fused = layer_norm.fused_available(cols);
    let case = BenchmarkCase::new("nnis_layernorm_fused_f32", "f32")
        .with_dimension("rows", rows as u64)
        .with_dimension("cols", cols as u64)
        .with_dimension("block_size", u64::from(layer_norm.block_size()))
    ...
    let report = benchmark_gpu(
        ...
        || {
            ...
            unsafe {
                layer_norm
                    .enqueue_fused_rows(&stream, &matrix, &output, rows, cols, EPSILON, GAMMA, BETA)
            }
        },
    )?;
    ...
    if actual.len() != rows * cols {
        return Err("layer norm output length mismatch".into());
    }
        ...
        .enumerate()
    {
        ...
        if (f64::from(actual) - expected).abs() > 1.0e-3_f64.max(expected.abs() * 1.0e-5) {
            return Err(format!("layer norm mismatch at {index}: {actual} != {expected}").into());
        }
    }

    let result = LayerNormBenchmark {
        schema_version: 1,
        report,
        rows,
        cols,
