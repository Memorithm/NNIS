...
fn parse_arguments() -> std::result::Result<Arguments, String> {
    ...
    while let Some(argument) = args.next() {
        match argument.as_str() {
            ...
            "--help" | "-h" => {
                return Err(
                    "usage: smollm2_f16_weight_allocation_accounting --model DIR [--device N]"
                        .to_string(),
                );
            }
...
fn run(arguments: Arguments) -> Result<Report> {
    ...
    Ok(Report {
        schema_version: 1,
        evidence: "nnis.f16-weight-allocation-accounting",
        measurement: "exact_resident_f16_weight_owned_cuMemAlloc_bytes_not_physical_page_residency",
        ...
        weight_allocations,
        limitations: vec![
            "counts only live DeviceBuffer allocations owned by the steady-state F16 model weight graph",
            "does not measure CUDA allocator metadata or physical GPU page residency",
            "does not include the dropped F32 source graph, transient KN narrowing buffers, or peak conversion memory",
