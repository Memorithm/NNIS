...
fn require_expected_profile(profile: &F16ReferenceGenerationProfile) -> Result<()> {
    if profile.generated_ids.as_slice() != EXPECTED_GREEDY_IDS {
        ...
        return Err(NnisError::invalid_input(format!(
            "F16 projection-plan trajectory changed at {divergence:?}: actual={:?} expected={:?}",
    ...
    {
        return Err(NnisError::invalid_input(format!(
            "F16 projection-plan metric contract drifted: {profile:?}"
        )));
    }
    Ok(())
}
...
fn run(arguments: Arguments) -> Result<Report> {
    ...
    let execution_plan = arguments.plan_name.execution_plan();
    ...
    let model = F16ReferenceModel::load_directory_with_execution_plan(
        &context,
        &construction_stream,
        &arguments.model_dir,
        execution_plan,
    ...
    if model.execution_plan() != execution_plan {
        return Err(NnisError::unsupported(
            "F16 model did not preserve the requested execution plan",
        ));
    }
    ...
    if model.execution_plan().projection_layout != expected_layout {
        return Err(NnisError::unsupported(
            "F16 model projection layout does not match requested plan",
        ));
    }
        ...
        if let Some(expected) = &generated_ids {
            if profile.generated_ids != *expected {
                return Err(NnisError::invalid_input(
                    "greedy output changed across measured F16 projection-plan iterations",
                ));
            }
        } else {
    ...
    if !gpu_median.is_finite() || gpu_median <= 0.0 {
        return Err(NnisError::unsupported(
            "F16 projection-plan generation GPU median is non-positive",
        ));
    }

    Ok(Report {
        schema_version: 1,
        benchmark: "smollm2-135m-f16-projection-plan-e2e",
        backend: "nnis",
        plan_name: arguments.plan_name.as_str(),
        promotion_state: "candidate comparison only; no runtime promotion from one process",
...
fn main() {
    ...
    match run(arguments) {
        Ok(report) => match serde_json::to_string_pretty(&report) {
            ...
            Err(error) => {
                eprintln!("failed to serialize F16 projection-plan report: {error}");
                std::process::exit(1);
            }
        },
        Err(error) => {
            eprintln!("SmolLM2 F16 projection-plan benchmark failed: {error}");
            std::process::exit(1);
        }
    }
}
