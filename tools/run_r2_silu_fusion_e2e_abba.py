#!/usr/bin/env python3
"""Run the R2 SiLU-multiply fusion end-to-end ABBA gate.

A is the promoted E1.1 all-f32 LM-head GEMV64 parent with the historical
separate SiLU and multiply launches. B keeps projection and representation
identical and changes only the explicit fusion plan to the physically evaluated
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run fingerprint-gated R2 SiLU fusion end-to-end ABBA evidence."
    )
...
def expected_fusion(variant: str) -> dict[str, Any]:
    if variant == "A":
        return PARENT_FUSION
    if variant == "B":
...
def validate_report(
    ...
    *,
    ...
    require(
        report.get("fusion_plan") == expected_fusion(variant),
        f"variant {variant} fusion plan mismatch",
    )
...
def output_directory(requested: Path | None) -> Path:
    if requested is None:
        ...
        requested = Path("artifacts") / f"r2-silu-fusion-e2e-abba-{stamp}"
...
def run_variant(
    *,
    ...
    fusion = "baseline" if variant == "A" else "r2-silu-multiply-fused"
    command = [
        "cargo",
        "run",
        "--locked",
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"R2 e2e variant {variant} failed:\n{completed.stderr.strip()}\n{completed.stdout.strip()}"
        )
    try:
...
def main() -> int:
    ...
    try:
        ...
        if dirty:
            raise RuntimeError("tracked worktree is dirty; R2 ABBA requires a clean exact head")
        ...
        for round_index in range(args.rounds):
            for sequence_index, variant in enumerate(("A", "B", "B", "A"), start=1):
                print(
                    f"R2 SiLU fusion e2e ABBA round {round_index + 1}/{args.rounds}: {variant}",
                    file=sys.stderr,
                    flush=True,
                )
        ...
        summary = {
            "schema_version": 1,
            "campaign": "R2-smollm2-silu-multiply-fusion-e2e-abba",
            ...
            "comparison": {
                ...
                / candidate_request,
                "all_candidate_generation_medians_below_all_parent_medians": all(
                    candidate_sample["generation_median_ms"]
                    < parent_sample["generation_median_ms"]
                    for candidate_sample in candidate
                    for parent_sample in parent
                ),
                "generation_separation_min_parent_minus_max_candidate_ms": (
                    min_parent_generation - max_candidate_generation
                ),
            },
            "semantic_gate": {
            ...
            "structural_change": {
                "launches_removed_per_layer": 1,
                "smollm2_layers": 30,
                "launches_removed_per_token": 30,
                "logical_intermediate_bytes_avoided_per_layer": 12_288,
                "logical_intermediate_bytes_avoided_per_token": 368_640,
                "activated_workspace_f32_elements_removed_per_session": 1_536,
                "activated_workspace_logical_bytes_removed_per_session": 6_144,
                "note": "structural/logical accounting; not measured DRAM traffic or allocator-resident bytes",
            },
            "decision_rule": (
                "Do not promote automatically. MinLatency requires a fingerprint-compatible "
                "end-to-end win credible relative to ABBA variation and unchanged greedy semantics."
            ),
            "samples": samples,
        }
        ...
        print(f"R2 SiLU fusion e2e ABBA failed: {error}", file=sys.stderr)
        return 1
