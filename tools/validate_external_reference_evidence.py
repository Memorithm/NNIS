...
def validate(data: dict[str, Any]) -> None:
    require(data.get("schema_version") == 1, "unsupported external reference evidence schema")
    require(data.get("kind") == "nnis_external_reference_evidence", "unexpected evidence kind")
    ...
    require(model.get("tokenizer_byte_identical_to_nnis_fixture") is True, "tokenizer identity gate did not pass")
    ...
    require(all(isinstance(token, int) and token >= 0 for token in prompt_ids), "prompt_ids must be non-negative integers")
    ...
    require(workload.get("apply_chat_template") is False, "R1 workload must not apply a chat template")
    require(workload.get("add_generation_prompt") is False, "R1 workload must not add a generation prompt")
    require(workload.get("context_cache_lookup_policy") == "bypass", "R1 workload must bypass context reuse")
    ...
    require(isinstance(reference_ids, list) and isinstance(nnis_ids, list), "generated token trajectories are required")
    require(len(reference_ids) == decode_steps, "reference generated-token count does not match decode_steps")
    require(len(nnis_ids) == decode_steps, "NNIS generated-token count does not match decode_steps")
    require(all(isinstance(token, int) for token in reference_ids + nnis_ids), "generated token ids must be integers")
    ...
    require(
        isinstance(semantic.get("nnis_reports_checked"), int) and semantic["nnis_reports_checked"] > 0,
    ...
    require(isinstance(edge_dtype, str) and edge_dtype, "Edge-LLM runtime dtype is required")
    require(isinstance(nnis_dtype, str) and nnis_dtype, "NNIS logical execution dtype is required")
    ...
    require(isinstance(processes, int) and processes > 0, "campaign process count must be positive")
    require(
        isinstance(campaign.get("warmups_per_process"), int) and campaign["warmups_per_process"] >= 0,
        "warmup count must be non-negative",
    )
    require(campaign.get("all_semantic_outputs_equal") is True, "performance campaign semantic outputs drifted")
    ...
    for key in (
        ...
        require(isinstance(semantics.get(key), str) and semantics[key].strip(), f"missing metric semantics for {key}")

    rows = campaign.get("raw_runs")
    require(isinstance(rows, list) and len(rows) == processes, "raw run count does not match processes")
    require([row.get("run") for row in rows] == list(range(1, processes + 1)), "raw run ordinals are incomplete or reordered")
    ...
    for row in rows:
        ...
        require(
            close(float(row["generation_tokens_per_second_nvidia_definition"]), expected_tps),
            "NVIDIA generation throughput does not match its recorded metric definition",
        )
        require(
            close(float(row["generation_average_time_per_token_ms_nvidia_definition"]), expected_average_ms),
    ...
    for key, recorded in ranges.items():
        ...
        require(isinstance(recorded, list) and len(recorded) == 2, f"range.{key} must contain min/max")
        require(close(float(recorded[0]), min(values[key])), f"range.{key} minimum does not match raw runs")
        require(close(float(recorded[1]), max(values[key])), f"range.{key} maximum does not match raw runs")
...
def main() -> int:
    ...
    print(f"external reference evidence accepted: {path}")
    return 0
